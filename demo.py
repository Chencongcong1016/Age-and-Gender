...
if __name__ == "__main__":
    ...
    for i, sample in enumerate(samples):
        ...
        if len(img.shape) == 3 and img.shape[-1] == 3:
            ...
            print(f"转置后img的新形状 (2, 0, 1): {img.shape}")
        else:
            print("img的形状不兼容(H, W, C)格式。")
        ...
        if len(img.shape) == 3 and img.shape[0] == 3:
            ...
            print(f"转置后img的新形状 (0, 2, 1): {img.shape}")

        # 如果 img 的形状是 (H, C, W)，我们使用 transpose(1, 0, 2)
        if len(img.shape) == 3 and img.shape[1] == 3:
            img = img.transpose(1, 0, 2)
            print(f"转置后img的新形状 (1, 0, 2): {img.shape}")
        # img = img.transpose(2, 0, 1)
        ...
        print(f"sample_preds: {sample_preds}")

    #在 Windows 上拼接路径有问题，具体原因还不清楚
